...
| 8 | Branchement backend Supabase (auth, tables sondages/entraide, storage, push) | 🟡 En cours (Steps 1-2 faits) | — |
...
**Limite de vérification (Steps 1 et 2)** : `.env` étant renseigné, le preview navigateur affiche l'écran de login réel et je n'ai pas de vrai compte résident pour un aller-retour visuel complet. `important_dates`/`appels_de_fonds` ont 0 lignes actuellement et `rent_payments` (8 lignes réelles) n'est pas encore rattaché à un vrai compte auth — le câblage est donc correct mais invisible tant qu'il n'y a pas de vrais comptes résidents liés, les écrans retombent sur les fixtures via le pattern de fallback déjà en place. Vérifié par `tsc`/lint/tests + simulation SQL des jointures des nouvelles policies contre les vraies lignes.
