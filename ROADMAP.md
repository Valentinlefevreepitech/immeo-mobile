...
| 8 | Branchement backend Supabase (auth, tables sondages/entraide, storage, push) | 🟡 En cours (Steps 1, 2, 3a, 3b faits) | — |
...
**Phase 8 — Step 3b fait (13/09/2026)** : câblage de Documents + correctif de sécurité. Trois tables se chevauchaient (`documents` niveau copropriété, `apartment_documents` en pratique vide/0 ligne, `tenant_documents` — **36 lignes réelles**, la vraie table utilisée pour bail/état des lieux/assurance/identité/RIB/avis d'imposition). Son champ `expiry_date` est exactement ce qu'il fallait pour rendre réelle la logique "attestation d'assurance à renouveler", jusque-là un texte statique.

**Le vrai sujet** : `tenant_documents` et `apartment_documents` avaient une policy SELECT `qual: true` — n'importe quel utilisateur connecté pouvait lire les documents (pièces d'identité, avis d'imposition, RIB...) de n'importe quel locataire. **Le bucket de stockage privé `tenant-documents` avait la même policy trop ouverte au niveau fichier** (`bucket_id = 'tenant-documents'` sans restriction) — corriger seulement la table n'aurait pas suffi. Fait important découvert en creusant : ces policies ouvertes étaient **le seul moyen pour le staff (app sœur) de lire ces documents** (aucune policy SELECT scopée par cabinet n'existait à côté, contrairement à INSERT/UPDATE/DELETE déjà bien scopées) — migration en deux temps : ajout des policies de remplacement (staff via cabinet, résident via ses propres documents, vérifié 36/36 lignes couvertes), puis suppression des 3 policies grandes ouvertes (2 tables + 1 bucket storage). Contrairement aux migrations précédentes, celle-ci n'était donc pas purement additive — confirmation explicite demandée et obtenue avant application.

Côté mobile : nouveau `hooks/useDocuments.ts` (aucun hook n'existait pour cet écran). Téléchargement réel via URL signée (`createSignedUrl`, bucket privé) pour les documents locataire, URL publique directe pour les documents copropriété (bucket public). "Mon lot" (rôle copropriétaire) reste sur fixture — `owner_documents` existe mais référence une table `owners` différente de `coproprietaires`, hors du modèle de rôle choisi au Step 1. "Déposer un document" reste un placeholder (un vrai flow d'upload est un chantier à part).

**Hors scope pour ce Step 3** (AG) : aucune table de vote individuel côté DB, seulement des compteurs agrégés sur `resolutions` — voter demande une nouvelle table, pas juste du câblage.
...
✅ **Partiellement fait** (12/09/2026), **onboarding refondu** (13/09/2026).
...
- **Connexion Google** : reste un placeholder ("bientôt disponible") — nécessite un vrai projet Supabase + des identifiants OAuth Google que seul l'utilisateur peut fournir.
- **Mot de passe oublié** : reste aussi un `Alert.alert` placeholder, pas touché.

**Refonte onboarding (13/09/2026)** — suite à des maquettes générées à partir d'un prompt de design (parcours en 8 écrans), `copro-setup.tsx` entièrement réécrit avec une vraie recherche/confirmation au lieu d'un rattachement automatique silencieux. Trois tensions tranchées avec l'utilisateur avant l'implémentation :
- **Code immeuble** : nouvelle colonne `coproprietes.invite_code`, dédiée à l'app — pas le digicode physique `code_acces` (sémantiquement faux à réutiliser, déjà écarté le 12/09).
- **"Créer ma copropriété"** : reste un mock local (nom/adresse/rôle → code généré affiché, pas persisté) — un immeuble vraiment autonome sans cabinet de gestion ne rentre pas proprement dans le schéma multi-tenant actuel, hors scope.
- **Rejoindre par code sans correspondance email** : le rôle est demandé après le code. Un copropriétaire crée sa fiche directement ; un locataire doit d'abord choisir son appartement dans une liste (`tenants.apartment_id` est obligatoire en base — pas d'autre choix).

Nouveau parcours dans `copro-setup.tsx` : recherche animée (checklist, appelle `authStore.searchMatch()` → RPC `find_resident_match_by_email`, lecture seule) → si trouvé, carte de confirmation (immeuble/lot/statut/syndic réels) avec "C'est bien mon logement" (`confirmMatch()` → `claim_resident_by_email`, la vraie action) / "Ce n'est pas ma copropriété" → si aucune correspondance, choix hiérarchisé code immeuble (conseillé) vs création, plus une sortie "continuer sans copropriété" (nouveau `authStore.onboardingSkipped`, respecté par `useAuthGuard`) → code immeuble (`joinByCode()`) → rôle → fiche minimale (`joinAsCoproprietaire()`/`joinAsLocataire()`, nouvelles policies `*_insert_self`) → écran de bienvenue commun.

Migration additive : `coproprietes.invite_code` + policy de lookup, `coproprietaires_insert_self`/`tenants_insert_self`, RPC `find_resident_match_by_email` (préview, ne modifie rien) et `list_copropriete_apartments` (projection volontairement minimale — id/numéro/étage seulement, pour ne jamais exposer `loyer`/`tenant_name`/`tenant_phone` à quelqu'un pas encore rattaché). `claim_resident_by_email` (Step 1) n'est pas modifiée, juste appelée à un autre moment (confirmation explicite, plus à l'inscription).
